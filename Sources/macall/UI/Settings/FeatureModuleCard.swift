    ...
    static func meta(_ id: String) -> FeatureMeta {
        ...
        case "mouseOptimize":
            return FeatureMeta(
                ...
                colors: IadenteTheme.dashboardColors,
                subtitle: IadenteL10n.t(
                    "鼠标滚轮独立反转、MOS 同款平滑引擎（滚动步长 / 速度 / 时长三参数）；只改鼠标、绝不碰触控板，默认全关。光标加速度由系统设置控制。",
                    "Invert the wheel and MOS-style smooth scrolling (step / speed / duration) — mouse only, never the trackpad, all off by default. Cursor acceleration is set in System Settings."),
                hotkeyPrefixes: [])
