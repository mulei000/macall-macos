...
enum WindowEnumerator {
    ...
    static func listWindows() -> [VSSwitcherItem] {
        let windowlessApps = SwitcherWindowlessApps.mode(
            storedValue: UserDefaults.standard.string(forKey: VSDefaultsKey.switcherWindowlessApps))
        return listWindows(filterPID: nil,
                           maximumCount: maximumCount,
                           windowlessApps: windowlessApps,
                           groupByApp: UserDefaults.standard.bool(forKey: VSDefaultsKey.switcherMergeTabs),
                           currentSpaceOnly: UserDefaults.standard.bool(forKey: VSDefaultsKey.switcherCurrentSpaceOnly))
    }

    static func listWindows(for pid: pid_t, maximumCount: Int = 12) -> [VSSwitcherItem] {
    ...
    private static func listWindows(filterPID: pid_t?,
                                    ...
                                    groupByApp: Bool,
                                    currentSpaceOnly: Bool) -> [VSSwitcherItem] {
        ...
        regularApps[pid_t(ownPid)] = VSAppInfo.name
        ...
        func isOnHiddenSpace(_ windowID: CGWindowID) -> Bool {
            ...
            if visibleSpaces == nil {
                visibleSpaces = SpaceWindowBridge.topology() ?? []
            }
    ...
    private static func accessibilityWindows(for pids: Set<pid_t>,
        ...
        guard VSPermissions.shared.accessibility else { return [:] }
    ...
    private static func appendAccessibilityOnlyWindows(to windows: inout [VSSwitcherItem],
    ...
    private static func appendWindowlessApps(to windows: inout [VSSwitcherItem],
        ...
        let chosen = SwitcherSupport.windowlessAppPIDs(
            ...
            desktopAppBundleIdentifier: VSDefaults.finderBundleIdentifier)
        for pid in chosen {
    ...
    private static func ownWindowTitle(for windowID: CGWindowID) -> String? {
        ...
        return window.title.isEmpty ? VSAppInfo.name : window.title
    }
    ...
    private static func orderByUse(_ windows: [VSSwitcherItem],
                                   frontToBack: WindowUseTracker.FrontToBack) -> [VSSwitcherItem] {
        // MRU ordering (WindowUseOrder) is adapted away in the Macindow port;
        // windows keep their enumeration order. Revisit in the joint refinement
        // step if recent-use ordering is desired.
        windows
    }
    ...
    private static func groupWindowsByApp(_ windows: [VSSwitcherItem]) -> [VSSwitcherItem] {
        var indexByPid: [pid_t: Int] = [:]
        var grouped: [VSSwitcherItem] = []
        for window in windows {
