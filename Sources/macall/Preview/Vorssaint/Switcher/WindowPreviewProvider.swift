...
final class WindowPreviewProvider {
    ...
    /// Read-only cache probe so callers can decide whether a fresh capture is
    /// worthwhile without forcing a capture on a cache miss. Returns `nil` when
    /// there's no entry or it predates `maxAge`. `cachedPreview` is intentionally
    /// not used here: it also refreshes `lastTouched`, which would keep a
    /// thumbnail alive purely because the switcher re-rendered it.
    func cacheEntry(for windowID: CGWindowID, maxAge: TimeInterval) -> CGImage? {
        guard let image = cache[windowID] else { return nil }
        let touched = lastTouched[windowID] ?? 0
        guard ProcessInfo.processInfo.systemUptime - touched <= maxAge else { return nil }
        return image
    }

    /// Stores a capture in the LRU cache (kept in sync with `cacheEntry`).
    func storeCached(_ image: CGImage, for windowID: CGWindowID) {
    ...
    func refreshPreviews(for items: [VSSwitcherItem],
                         maxPixelSize: CGFloat = defaultMaxPixelSize,
                         onUpdate: @escaping (CGWindowID, CGImage) -> Void) {
        guard VSPermissions.shared.screenRecording, !Self.captureIsPaused else {
            cancel()
            return
        }
    ...
    private func scheduleWarm(pid: pid_t) {
        guard VSPermissions.shared.screenRecording, !Self.captureIsPaused else { return }
        pendingWarmPid = pid
    ...
    private static var captureIsPaused: Bool {
        let excluded = VSDefaults.sanitizedBundleIdentifierList(
            UserDefaults.standard.stringArray(forKey: VSDefaultsKey.windowPreviewExcludedApps) ?? [])
